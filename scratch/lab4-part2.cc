...
void
RxDrop(Ptr<const Packet> packet, WifiPhyRxfailureReason reason)
{
    nPackets++;
}
void moveStation(Vector pos, uint32_t step, Ptr<Node> stNode)
{
    ...
    std::cout << "Time: " << Simulator::Now().GetSeconds() << "s, "
            ...
            << "Number of rate changes: "<< nRateChanges << " "
            << "Number of packets dropped: "<< nPackets << std::endl;
    nPackets = 0;
    nRateChanges = 0;
    Vector nextPos = pos;
    if (pos.y < 20)
...
main(int argc, char *argv[])
{
    ...
    Config::ConnectWithoutContext(
        ...
        MakeCallback(RxDrop));
    packetSink = DynamicCast<PacketSink>(sinkApp.Get(0));
